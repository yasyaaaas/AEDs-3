...
public class Arquivo<T extends Registro> {
  ...
  public int create(T obj) throws Exception {
    ...
    arquivo.seek(acharLapide(tam)); // chamada da função criada
  ...
  public long EncontrarCampoLapide(short tamanho){
    arquivo.seek(TAM_CABECALHO); // começa a leitura a partir do final do cabeçalho 

    long temp; // armazena o endereço do espaço vazio(lapide)
    short tamanho_tmp; // variavel para guardar o tamanho do registo

    while (arquivo.getFilePointer() < arquivo.length()) { // continua lendo o arquivo até chegar ao final do mesmo
      temp = arquivo.getFilePointer(); // a cada repetição atualiza o valor de temp com o endereço atual

      if (arquivo.readByte() == '*') {
        tamanho_tmp = arquivo.readShort(); // caso encontre uma lápide ( * ) ler o tamanho do registro  

        if (tamanho_tmp >= tamanho) { 
          return temp; // se o tamanho do registro for suficiente para armazenar o conteúdo requisitado, retorna seu endereço
        } else {
          arquivo.skipBytes(tamanho_tmp); // caso contrário pula os bytes correspondeste ao registro lápide e continua o processo ate o fim do while
        }

      }

    }
    return arquivo.length();// caso o while não encontre espaço suficiente para escrever, retorna o final do arquivo.
  ...
  public boolean update(T novoObj) throws Exception {
    ...
    if (endereco != -1) {
      ...
      if (tam2 <= tam) {
        ...
        arquivo.seek(acharLapide(tam2)); // chamada da função criada
